...
public class SoilNoteDB {
	...
	/*
	 * 将构造方法私有化
	 * 在实例化这个类的时候，也同时实例化SoilNoteOpenHelper类，并且创建好数据库
	 */
	private SoilNoteDB(Context context){
		SoilNoteOpenHelper dbHelper = new SoilNoteOpenHelper(context, DB_NAME, null, VERSION);//实例化
		dbHelper.getWritableDatabase();//创建数据库中的表
	}
	...
	public List<InfoGeo> loadInfoGeo() {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				infoGeo.setLatitude(cursor.getDouble(cursor.getColumnIndex("latitude")));
				infoGeo.setLongtitude(cursor.getDouble(cursor.getColumnIndex("longtitude")));
				infoGeo.setPosition(cursor.getString(cursor.getColumnIndex("position")));
				list.add(infoGeo);
